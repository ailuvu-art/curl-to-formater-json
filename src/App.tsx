...
async function executeCurl(curl: string, mode: ExecutionMode): Promise<ResponseResult> {
  ...
  if (mode === 'agent') {
    ...
    try {
      ...
      throw new Error(`Cannot reach the Local Agent at ${getAgentUrl()}. Start it with npx @nolann-dev/local-agent start.`)
    }
...
function AgentSetup({ onConnected }: { onConnected: (status: AgentStatus) => void }) {
  ...
  return <div className="agent-setup">
    <div><strong>Connect Local Agent</strong><span>Run <code>npx @nolann-dev/local-agent install</code>, then paste the printed token.</span></div>
